...
class OneShotRecallTaskOmniglotParams(TaskBaseParams):
    ...
    variationalmodel = VariationalModelConvDeconvOmniglot
    ...
    def load_MNIST(self):
        # train data set
        train_dataset = datasets.Omniglot('data',
                                          download=True,
                                          background=True,
                                          transform=transforms.ToTensor())

        # test data set
        test_dataset = datasets.Omniglot('data',
                                          download=True,
                                          background=False,
                                          transform=transforms.ToTensor())
    ...
    def generate_random_batch(self, device='cpu', train = True):
        ...
        for i in range(self.sequence_l):
            ...
            _data = _data.squeeze()
            _data = _data.view(-1, 105 * 105)
            _data = 1 - _data # invert black and white
    ...
    def create_images(self, batch_num, X, Y, Y_out, Y_out_binary, attention_history, modelcell):
        ...
        # save images
        _X = torch.cat([X[i].view(105,105) for i in range(X.size(0))], dim = 1)
        torchvision.utils.save_image(_X.cpu().detach().squeeze(1),
                                     '{}/batch-{}-X.png'.format(path, batch_num))

        _Y = torch.cat([Y[i].view(105,105) for i in range(Y.size(0))], dim = 1)
        torchvision.utils.save_image(_Y.cpu().detach().squeeze(1),
                                     '{}/batch-{}-Y.png'.format(path, batch_num))

        _Y_out = torch.cat([Y_out[i].view(105,105) for i in range(Y_out.size(0))], dim = 1)
        ...
        _Y_out_binary = torch.cat([Y_out_binary[i].view(105, 105) for i in range(Y_out_binary.size(0))], dim=1)
